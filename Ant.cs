...
public class Ant : MonoBehaviour
{
    ...
    // Kundali data
    private string backendUrl = "http://localhost:5000/calculate_kundali";
    private Dictionary<string, float> kundaliData;
    ...
    void Start()
    {
        ...
        // Fetch Kundali data at the start
        StartCoroutine(FetchKundaliData("1990-05-15", "12:00", "New York"));
    }
    ...
    IEnumerator FetchKundaliData(string birthDate, string birthTime, string birthPlace)
    {
        // Create JSON payload
        var payload = new
        {
        ...
        using (UnityWebRequest webRequest = new UnityWebRequest(backendUrl, "POST"))
        {
            ...
            if (webRequest.result == UnityWebRequest.Result.Success)
            {
                ...
                Debug.Log("Kundali Data: " + responseText);
                // Parse the response and store it in kundaliData
                kundaliData = JsonUtility.FromJson<Dictionary<string, float>>(responseText);
            }
            else
            {
                Debug.LogError("Error fetching Kundali data: " + webRequest.error);
            }
        }
    }
}